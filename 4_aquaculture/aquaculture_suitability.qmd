...
# Vectorize regions & mask to ocean regions
regions_vect <- vect(wc_regions_sf)
suitable_all_mask <- mask(suitable_sst_depth, regions_vect)
```
...
region_counts <- terra::extract(suitable_all_mask, wc_regions_sf, touches = TRUE) |> 
  group_by(ID) |> 
...
area_raster <- suitable_all_mask * cellSize(suitable_all_mask, unit = 'km')
```
...
To find suitable temperatures for oysters, a reclassification matrix was used to assign temperature cells to 1 if they *are suitable*, and to 0 if *not suitable*. In the same way, suitable depth cells were identified. Multiplying the suitable rasters resulted in a new raster that indicated suitable cells for both temperature and depth parameters. The next step was assign cells = 0 to NA and mask these cells for area calculations of suitable cells within EEZ regions only. To account for the varying cell size in EPSG:4326, the function `terra::cellSize()` was used and multiplied to our suitable cells mask. Extracting these cells from our ocean regions and aggregating, we're left with a dataframe with regions and their respective suitable area for aquaculture calculation. Joining back with the original regions dataframe, the geometries can be plotted by each region's suitable area in $\text{km}^2$. According to the map, the region with the least oyster potential is Northern California and the most is Central California.

Next, a generalized function can be created to do this for any species of choice, given suitable temperature and depth parameters.
...
map <- tm_shape(region_suitable_area) +
  tm_basemap("CartoDB.VoyagerNoLabels") +
  tm_polygons(fill = "suitable_area", 
...
Using the generalized workflow, a map showing suitable area ($\text{km}^2$) for rock scallop aquaculture was created. Compared to oysters, rock scallops have a narrower temperature range and prefer shallower waters. Indicated by the green gradient, Southern California has the most suitable area for scallop aquaculture followed by Central California.
