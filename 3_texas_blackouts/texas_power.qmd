...
tm_shape(blackout_far_areas) + # Blackout far areas
...
```{r}
# Summary statistics
model <- lm(median_hh_income_est ~ blackout, data = census_blackout_tf)
summary(model)
```

The boxplots in @fig-income don't give expected results. 
...
There are twice more blackout census tracts than non-blackout. Census tracts with blackouts could be biasing the data. 
