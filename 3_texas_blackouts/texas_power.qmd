...
# Add total row
homes_df <- homes_df |> 
  add_row(type = 'Total', count = sum(homes_df$count), percent = NA) 
...
# # Extract house layer
# house_count <- st_read(here::here('data', 'ACS_2019_5YR_TRACT_48_TEXAS.gdb'),
#                   layer = 'X11_HOUSEHOLD_FAMILY_SUBFAMILIES', quiet = TRUE) |> 
#   select(c("GEOID", "B11001e1"))
...
# Plot blackout homes with census blackouts
...
# Spatially join census_income data with only tracts that experienced blackouts
...
The boxplots in @fig-income don't give expected results. Will try to weigh the median_income column in the census data by number of households.

```{r}
# Get value counts for blackout column
table(census_spatjoin$blackout)
```

There are twice more blackout census tracts than non-blackout. Census tracts with blackouts seem to be biasing the data. 

```{r}
# Keep only homes within Houston tracts
homes_houston <- st_join(homes, census_income, join = st_within)

# Find number of homes in census tracts
homes_in_tracts <- homes_houston |> 
  st_drop_geometry() |> 
  group_by(GEOID) |> 
...
```{r}
# Join with Houston census blackout dataframe
census_houses <- left_join(census_spatjoin, homes_in_tracts, by = "GEOID")
```

```{r}
ggplot(census_houses, aes(blackout, median_hh_income_est, weight = home_count)) +
  geom_boxplot()
```
