...
census_income <- left_join(x = census, y = income, by = "GEOID") |> 
  st_transform(st_crs(houston_bbox)) |> # FIXME, use 1 crs throughout qmd
...
census_blackouts <- census_income |>
  st_transform(st_crs(homes_blackout)) |> 
  st_filter(y = homes_blackout, .predicate = st_contains) |> 
...
income_blackouts <- census_income |> 
  left_join(census_blackouts |> 
            ...
            by = "GEOID") |> 
  mutate(blackout = ifelse(is.na(blackout), FALSE, blackout))
```
