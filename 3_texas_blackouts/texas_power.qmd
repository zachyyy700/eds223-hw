...
homes_blackout <- homes |> # FIXME: number of obs in homes_blackout are too low
  st_filter(y = st_union(blackout_far_areas), .predicate = st_overlaps) # Try st_union
```

```{r}
# Double check blackout_far_areas
tm_shape(st_union(blackout_far_areas)) +
  tm_polygons()
```
...
```{r}
# Rejoin to census_income & update blackouts column
# FIXME
income_blackouts <- census_income |> 
  left_join(census_blackouts |> 
              st_drop_geometry() |> 
              select(c("GEOID", "blackout")),
            by = "GEOID") |> 
  mutate(blackout = ifelse(test = is.na(blackout), yes = FALSE, no = blackout))
```

```{r}
# Rejoin with second method
# FIXME

# Drop geoms first
census_blackouts_df <- census_blackouts |> st_drop_geometry()
test <- full_join(census_income, census_blackouts_df) |> 
  mutate(blackout = ifelse(test = is.na(blackout), yes = FALSE, no = blackout)) # Fill NAs with FALSE
...
ggplot(income_blackouts, aes(blackout, median_hh_income_est)) +
  geom_boxplot()
```
...
```{r}
# Summary statistics
model <- lm(median_hh_income_est ~ blackout, data = income_blackouts)
summary(model)
```
